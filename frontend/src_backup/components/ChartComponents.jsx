...
export function HourlyTrafficChart({ trafficData }) {
  return (
    <div className="glass-panel equal-height-card" style={{ padding: '20px', minHeight: '300px' }}>
...
export function ModeDistributionChart({ modeData }) {
  ...
  return (
    <div className="glass-panel equal-height-card" style={{ padding: '20px', minHeight: '300px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: '16px', alignItems: 'center', marginTop: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {data.map((item) => (
            <div key={item.mode} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.82rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ color: 'var(--text-main)', fontWeight: 600 }}>{item.mode}</span>
              </div>
