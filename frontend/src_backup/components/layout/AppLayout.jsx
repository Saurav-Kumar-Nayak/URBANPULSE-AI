...
export const AppLayout = ({ children }) => {
  const { activeTab, toggleCopilot, isAuthenticated, isEvidenceModalOpen, closeEvidenceModal, selectedInsightEvidence } = useUrbanPulseContext();
  ...
  return (
    <div className="app-layout" style={{ minHeight: '100vh', background: '#070b12', color: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Global Statistical Evidence Modal */}
      <EvidenceModal 
        isOpen={isEvidenceModalOpen} 
        onClose={closeEvidenceModal} 
        insight={selectedInsightEvidence} 
      />
    </div>
  );
};
